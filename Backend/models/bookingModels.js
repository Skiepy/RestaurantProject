...
export const updateBookingById = (data, id, result) => {
    db.query("UPDATE dbresto.booking SET users_id = ?, firstname = ?, lastname = ?, passport = ?, age = ?, occupation = ?, citizenship = ?, nbPeople = ?, menu = ?, date = ?, timeSlot = ?, allergies = ?, diet = ?, smoking = ? WHERE booking_id = ?", [data.users_id, data.firstname, data.lastname, data.passport, data.age, data.occupation, data.citizenship, data.nbPeople, data.menu, data.date, data.timeSlot, data.allergies, data.diet, data.smoking, data.booking_id], (err, results) => {
        if (err) {
            console.log(err);
            result(err, null);
        } else {
